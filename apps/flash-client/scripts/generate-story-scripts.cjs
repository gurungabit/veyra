...
const outputDir = path.resolve(__dirname, "../src/scripts/Story");
const legacyOutputFile = path.resolve(__dirname, "../src/scripts/Story/GeneratedStories.ts");
const legacyOutputDir = path.resolve(__dirname, "../src/scripts/Story/Generated");
const generatedHeader = "// @generated by scripts/generate-story-scripts.cjs\n// Do not edit by hand. Run pnpm stories:generate.\n\n";
...
function main() {
  ...
  fs.rmSync(legacyOutputDir, { force: true, recursive: true });
...
function removeGeneratedFiles(directory) {
  ...
  let removedGeneratedFile = false;
  for (const name of fs.readdirSync(directory)) {
    const filePath = path.join(directory, name);
    const stats = fs.statSync(filePath);
    if (stats.isDirectory()) {
      const removedInChild = removeGeneratedFiles(filePath);
      removedGeneratedFile = removedGeneratedFile || removedInChild;
      if (removedInChild && fs.readdirSync(filePath).length === 0) fs.rmdirSync(filePath);
      continue;
    }
    if (!stats.isFile() || !name.endsWith(".ts")) continue;
    const text = fs.readFileSync(filePath, "utf8");
    if (!text.startsWith(generatedHeader)) continue;
    fs.rmSync(filePath);
    removedGeneratedFile = true;
  }

  return removedGeneratedFile;
}
...
function renderGeneratedStoryModule(entry, runnerImport) {
  return `${generatedHeader}import { defineGeneratedStory, type StoryStep } from "${runnerImport}";
...
function renderGeneratedIndex(entries) {
  ...
  return `${generatedHeader}import type { GeneratedStoryDefinition } from "./runner.js";
