...
import { FarmJoeRuntime, isClassItem, itemName, type FarmJoeRuntimeOptions, type PetChoice } from "./FarmJoeRuntime.js";

export const meta = {
  name: "CoreFarmerJoe",
  description: "Veyra conversion of CoreFarmerJoe.cs progression, class setup, outfit, pet, and endgame flow.",
  tags: ["farmjoe", "progression", "core", "converted"],
  version: "0.1.0"
};
...
const farmClasses = ["Legion Revenant", "ArchMage", "Dragon of Time", "Archfiend", "Blaze Binder", "Scarlet Sorceress", "Pyromancer", "Master Ranger", "Mage (Rare)", "Mage"];
const dodgeClasses = ["Great Thief", "Yami No Ronin", "Chrono Assassin", "Horc Evader", "Assassin", "Ninja Warrior", "Classic Ninja", "Ninja", "Ninja (Rare)", "Rogue (Rare)", "Rogue"];

export class CoreFarmerJoe {
  ...
  async doAll(): Promise<void> {
    await this.runtime.externalStep("CoreAdvanced.GearStore", "initial gear save and enhancement pass");
    ...
    await this.runtime.externalStep("CoreAdvanced.GearStore", "final gear save and enhancement pass");
  }

  async level1to30(): Promise<void> {
    await this.beginnerItems();
    this.runtime.log("Class points may desync around Rank 9; if a class appears stuck, stop and relog before resuming.");
  }

  async level30to75(): Promise<void> {
    await this.setClass();
    if (await this.runtime.isQuestCompleted(802)) await this.runtime.externalStep("CoreDailies.EldersBlood");
    this.runtime.log("FarmJoe level brackets: 30, 50, 55, 60, 65, 70, 75.");
  ...
  async level75to100(): Promise<void> {
    ...
    else if (!(await this.runtime.contains(["Healer", "Healer (Rare)"], 1, true))) await this.runtime.buyItem("classhalla", 176, "Healer");

    this.runtime.log("Phase 2: Cape of Awe and 13 Lords of Chaos.");
    await this.runtime.externalStep("CapeOfAwe.GetCoA");
    await this.runtime.equip("Cape of Awe");
    await this.setClass();
    await this.runtime.externalStep("Core13LoC.Complete13LOC");
    ...
    for (const className of ["Frost Spirit Reaver", "Northlands Monk", "Shaman"]) {
      if (!(await this.runtime.contains(className))) await this.runtime.externalStep(`${className} acquisition`);
    ...
    await this.runtime.externalStep("CelestialArena.DoAll");
    await this.runtime.externalStep("Blinding Bright of Awe.GetBBoA");
    ...
    await this.runtime.externalStep("CoreYnR.GetYnR");
    await this.setClass();
    await this.runtime.externalStep("DragonOfTime.GetDoT");
    await this.runtime.farmExperience(100);
    await this.setClass();
    await this.runtime.externalStep("KingsEcho.GetKE");
    await this.runtime.externalStep("ShadowrealmMerge.BuyAllMerge", "Hollowborn Reaper's Scythe");
  ...
  async endGame(): Promise<void> {
    ...
    for (const enhancement of ["HerosValiance", "Elysium", "Ravenous"]) await this.runtime.unlockEnhancement(enhancement);
    await this.runtime.externalStep("ExaltedApotheosisPreReqs.PreReqs");
    this.runtime.log("P6: Max nation materials via Supplies.");
    await this.runtime.externalStep("CoreNation.Supplies");
  }
  ...
  async setClass(): Promise<void> {
    ...
    const unchanged = !levelIncreased && this.lastSolo === this.soloClass && this.lastFarm === this.farmClass && this.lastDodge === this.dodgeClass;
    if (unchanged) return;
  ...
  private async handleLevel30(): Promise<void> {
    if (!(await this.runtime.contains("Master Ranger")) || !(await this.runtime.anyRank10(["Master Ranger"]))) {
      if (await this.runtime.contains("Venom Head")) await this.runtime.sell("Venom Head");
      await this.runtime.externalStep("MasterRanger.GetMR");
    }
    if (!(await this.runtime.contains("Dragonslayer")) || !(await this.runtime.anyRank10(["Dragonslayer"]))) {
      await this.runtime.externalStep("Dragonslayer.GetDragonslayer");
    }
  ...
  private async handleLevel50(): Promise<void> {
    if (!(await this.runtime.contains("Scarlet Sorceress")) || !(await this.runtime.anyRank10(["Scarlet Sorceress"]))) await this.runtime.externalStep("ScarletSorceress.GetSSorc");
    if (!(await this.runtime.contains("Dragonslayer General")) || !(await this.runtime.anyRank10(["Dragonslayer General"]))) await this.runtime.externalStep("DragonslayerGeneral.GetDSGeneral");
    await this.runtime.externalStep("BurningBlade.GetBurningBlade");
  }

  private async handleLevel55(): Promise<void> {
    if (!(await this.runtime.contains("Blaze Binder")) || !(await this.runtime.anyRank10(["Blaze Binder"]))) await this.runtime.externalStep("BlazeBinder.GetClass");
    if (!(await this.runtime.contains("Cryomancer")) || !(await this.runtime.anyRank10(["Cryomancer"]))) await this.runtime.externalStep("Cryomancer.DoCryomancer");
  }

  private async handleLevel60(): Promise<void> {
    if (!(await this.runtime.contains("DragonSoul Shinobi")) || !(await this.runtime.anyRank10(["DragonSoul Shinobi"]))) await this.runtime.externalStep("DragonShinobi.GetDSS");
    await this.runtime.unlockEnhancement("Lacerate");
  }

  private async handleLevel65(): Promise<void> {
    if (!(await this.runtime.contains("Glacial Berserker")) || !(await this.runtime.anyRank10(["Glacial Berserker"]))) await this.runtime.externalStep("GlacialBerserker.GetGB");
  }

  private async handleLevel70(): Promise<void> {
    if (!(await this.runtime.contains("Horc Evader")) || !(await this.runtime.anyRank10(["Horc Evader"]))) await this.runtime.buyItem("bloodtusk", 308, "Horc Evader");
    for (const enhancement of ["Smite", "Vim", "Pneuma", "Examen", "Anima"]) await this.runtime.unlockEnhancement(enhancement);
    if (!(await this.runtime.contains("ArchPaladin")) || !(await this.runtime.anyRank10(["ArchPaladin"]))) await this.runtime.externalStep("ArchPaladin.GetAP");
  }

  private async handleLevel75(): Promise<void> {
    if (!(await this.runtime.contains("Archfiend DoomLord"))) await this.runtime.externalStep("EnoughDOOMforanArchfiend.AFDL");
    if (!(await this.runtime.contains("Archfiend")) || !(await this.runtime.anyRank10(["Archfiend"]))) await this.runtime.externalStep("Archfiend.GetArchfiend");
  }

  private async beginnerItems(): Promise<void> {
    const hasHighTier = (await this.runtime.contains(soloClasses.slice(0, -9), 1, true)) && (await this.runtime.contains(farmClasses.slice(0, -2), 1, true));
    ...
    await this.runtime.externalStep("Tutorial.Badges");
    ...
    if (!(await this.runtime.contains(["Rogue (Rare)", "Rogue"], 1, true))) await this.runtime.buyItem("classhalla", 172, "Rogue");
    if (!(await this.runtime.contains(["Assassin", "Ninja Warrior", "Ninja"], 1, true))) {
      ...
      await this.setClass();
      await this.runtime.externalStep("Mazumi.MazumiQuests");
      await this.runtime.buyItem("classhalla", 178, "Ninja");
    }
    await this.setClass();
    if (!(await this.runtime.contains(["Mage (Rare)", "Mage"], 1, true))) await this.runtime.buyItem("classhalla", 174, 15653, 1, 9845);
    await this.setClass();
  }

  private async shirtAndHat(): Promise<void> {
    await this.runtime.externalStep("SynderesMerge.BuyAllMerge", "NO BOTS Armor");
  ...
  private async resolveClass(current: string, pool: string[]): Promise<string> {
    if (current && current.toLowerCase() !== "generic" && pool.some((name) => name.toLowerCase() === current.toLowerCase()) && (await this.runtime.contains(current))) {
      return current;
    }
  ...
  private async bankAllUnusedClasses(): Promise<void> {
    const keep = new Set([this.soloClass, this.farmClass, this.dodgeClass].filter(Boolean).map((name) => name.toLowerCase()));
